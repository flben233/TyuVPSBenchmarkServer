...
const { listHosts, addHost, removeHost, listPendingHosts, approveHost, rejectHost } = useMonitor();
const { listRecords, addRecord, removeRecord, listPendingRecords, approveRecord, rejectRecord } = useLookingGlass();
...
async function handleAddHost() {
  ...
  loading.value = true;
  try {
    const result = await addHost(token.value, addHostForm.value.name, addHostForm.value.target);
    if (result.success) {
...
async function handleAddRecord() {
  ...
  loading.value = true;
  try {
    const result = await addRecord(token.value, addRecordForm.value.serverName, addRecordForm.value.testUrl);
    if (result.success) {
...
async function handleAddReport(event) {
  ...
  file.text().then(async (htmlContent) => {
    const result = await addReport(token.value, htmlContent);
    if (result.success) {
...
async function handleDeleteReport() {
  try {
    const { value: reportId } = await ElMessageBox.prompt('请输入报告 ID', '删除报告', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      inputPattern: /.+/,
...
<template>
  <el-row id="center-root">
        ...
        <el-tabs v-model="activeTab" class="center-tabs">
          ...
          <el-tab-pane label="监控主机" name="monitor">
            <el-card shadow="never">
              <template #header>
                <div class="card-header">
                  <span>我的监控主机</span>
                  <el-button type="primary" size="small" @click="addHostDialogVisible = true">
                    添加主机
                  </el-button>
                </div>
              </template>
              ...
              <el-table :data="hosts" v-loading="loading" border stripe>
                ...
                <el-table-column label="操作" width="120">
                  <template #default="{ row }">
                    <el-button type="danger" size="small" @click="handleRemoveHost(row.id)">
                      删除
                    </el-button>
                  </template>
                </el-table-column>
              ...
              <el-empty v-if="hosts.length === 0 && !loading" description="暂无主机" />
            </el-card>
            ...
            <el-card shadow="never" v-if="isAdmin" style="margin-top: 16px;">
              ...
              <el-table :data="pendingHosts" v-loading="loading" border stripe>
                <el-table-column prop="id" label="ID" width="80" />
                <el-table-column prop="name" label="名称" min-width="150" />
                <el-table-column prop="target" label="目标" min-width="200" />
                <el-table-column prop="uploader_name" label="上传者" min-width="120" />
                <el-table-column label="操作" width="200">
                  <template #default="{ row }">
                    <el-button type="success" size="small" @click="handleApproveHost(row.id)">
                      通过
                    </el-button>
                    <el-button type="danger" size="small" @click="handleRejectHost(row.id)">
                      拒绝
                    </el-button>
                  </template>
                </el-table-column>
              ...
              <el-empty v-if="pendingHosts.length === 0 && !loading" description="暂无待审核主机" />
            </el-card>
          </el-tab-pane>
          ...
          <el-tab-pane label="Looking Glass" name="lookingglass">
            <el-card shadow="never">
              <template #header>
                <div class="card-header">
                  ...
                  <el-button type="primary" size="small" @click="addRecordDialogVisible = true">
                    添加记录
                  </el-button>
                </div>
              </template>
              ...
              <el-table :data="records" v-loading="loading" border stripe>
                ...
                <el-table-column prop="test_url" label="测试 URL" min-width="200">
                  <template #default="{ row }">
                    <el-link :href="row.test_url" target="_blank" type="primary">
                      {{ row.test_url }}
                    </el-link>
                  </template>
                </el-table-column>
                ...
                <el-table-column label="操作" width="120">
                  <template #default="{ row }">
                    <el-button type="danger" size="small" @click="handleRemoveRecord(row.id)">
                      删除
                    </el-button>
                  </template>
                </el-table-column>
            ...
            <el-card shadow="never" v-if="isAdmin" style="margin-top: 16px;">
              ...
              <el-table :data="pendingRecords" v-loading="loading" border stripe>
                <el-table-column prop="id" label="ID" width="80" />
                <el-table-column prop="server_name" label="服务器名称" min-width="150" />
                <el-table-column prop="test_url" label="测试 URL" min-width="200">
                  <template #default="{ row }">
                ...
                <el-table-column label="操作" width="200">
                  <template #default="{ row }">
                    <el-button type="success" size="small" @click="handleApproveRecord(row.id)">
                      通过
                    </el-button>
                    <el-button type="danger" size="small" @click="handleRejectRecord(row.id)">
                      拒绝
                    </el-button>
                  </template>
                </el-table-column>
              ...
              <el-empty v-if="pendingRecords.length === 0 && !loading" description="暂无待审核记录" />
            </el-card>
          </el-tab-pane>
          ...
          <el-tab-pane label="报告管理" name="report" v-if="isAdmin">
            <el-card shadow="never">
              ...
              <el-space direction="vertical" style="width: 100%;" :size="16">
                <input type="file" ref="report-selector" style="display: none;" @change="handleAddReport"/>
                <el-button type="primary" @click="showFileSelector" :loading="loading">
                  添加报告
                </el-button>
    ...
    <!-- Add Host Dialog -->
    <el-dialog v-model="addHostDialogVisible" title="添加监控主机" width="500px">
      <el-form :model="addHostForm" label-width="100px">
        <el-form-item label="名称">
          <el-input v-model="addHostForm.name" placeholder="输入主机名称" />
        </el-form-item>
        <el-form-item label="目标">
          <el-input v-model="addHostForm.target" placeholder="输入目标（IP 或域名）" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="addHostDialogVisible = false">取消</el-button>
        <el-button type="primary" @click="handleAddHost" :loading="loading">添加</el-button>
      </template>
    </el-dialog>

    <!-- Add Record Dialog -->
    <el-dialog v-model="addRecordDialogVisible" title="添加 Looking Glass 记录" width="500px">
      <el-form :model="addRecordForm" label-width="120px">
        <el-form-item label="服务器名称">
          <el-input v-model="addRecordForm.serverName" placeholder="输入服务器名称" />
        </el-form-item>
        <el-form-item label="测试 URL">
          <el-input v-model="addRecordForm.testUrl" placeholder="输入测试 URL" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="addRecordDialogVisible = false">取消</el-button>
        <el-button type="primary" @click="handleAddRecord" :loading="loading">添加</el-button>
      </template>
    </el-dialog>
  </el-row>
</template>
